...
const FormGenerator = <T extends FieldValues>({
  ...
  const renderInput = () => (
    <Label
      className="flex flex-col justify-center gap-2 font-normal"
      htmlFor={`input-${id}`}
    >
      {label}
      <Input
  ...
  const renderSwitch = () => {
    ...
    const renderLabel = (label: string) => {
      const [mainText, bracketText] = label.split(/(\（.*\）)/);

      return (
        <span className="w-full flex-1 break-words">
          {mainText}
          {bracketText && (
            <span className="text-[rgba(128,128,128,1)]">{bracketText}</span>
          )}
        </span>
      );
    };
    ...
    return (
      <Label
        className="flex w-full items-center gap-4 font-normal"
        htmlFor={`switch-${id}`}
      >
        {renderLabel(label || "")}
        <Switch
